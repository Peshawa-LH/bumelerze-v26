/**
 * felt-aggregation IO surface.
 *
 * PORTABILITY CONSTRAINT: this entire folder (`src/lib/felt-aggregation/`)
 * is pure TypeScript — zero React/React Native/Expo imports, zero new npm
 * dependencies — so it can run unchanged inside a Supabase Edge Function
 * (Deno) once the server-side aggregation job is built (Phase 2 backend
 * wave). Keep it that way: no `@/features/felt/*` VALUE imports here, only
 * `import type` of the answer-enum unions, which erase at compile time and
 * leave zero runtime coupling to the RN app.
 *
 * Input types are intentionally minimal — this module does not know about
 * `Tier1Report`/`Tier2Report`'s full client bookkeeping (queue state,
 * timestamps, RN-only fields); callers adapt those into the
 * `AggregationInputReport` shapes below.
 */
...
export interface Tier2InputReport extends AggregationInputReportBase {
   ...
   * corroboration-gate evidence count in `cell-aggregation.ts`, which is
   * described in terms of raw per-report levels ≥8 and would otherwise be
   * blind to tier-2 respondents entirely. See that file's doc comment for
   * the full reasoning; this is a documented interpretation, not a value
   * pulled verbatim from the science pack.
   */
  cartoonLevel?: CartoonLevel;
}
...
export interface FeltCellAggregate {
   ...
   * already applied. `null` is unreachable in practice (callers only ever
   * get a `FeltCellAggregate` via the `ok: true` branch of
   * `aggregateCell`'s result, which guarantees ≥1 report), kept nullable
   * only so the type doesn't lie about a genuinely-empty cell. */
  cdi: number | null;
   ...
   * Below this, the cell exists (and `cdi` is still computed/stored) but
   * renders no intensity color. */
   ...
   * `null` otherwise (either too few total reports, or extreme evidence
   * without corroboration — see the "corroboration gate" doc comment in
   * `cell-aggregation.ts` for the load-bearing ambiguity this resolves). */
...
/** Result-style outcome (project convention: no thrown strings for
 * fallible-but-expected states) — `aggregateCell` is only ever asked to
 * aggregate a non-empty report list in practice, but an empty input is a
 * real possibility for a caller (e.g. a cell whose only reports were all
 * later retracted), so it is modelled, not thrown. */
