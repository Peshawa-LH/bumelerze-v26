 ...
 * bumelerze-engine/scripts/bundle_atlas_for_app.py — do not hand-edit.
 * Regenerate: ./.venv/bin/python scripts/bundle_atlas_for_app.py
 * (from bumelerze-engine/), after re-running seed_atlas.py.
 *
 * Maps every curated historical event id
 * (src/features/historical/notable-events.ts) to its bundled
 * bumelerze-engine product — raw cont_mi.json contours +
 * review_status + a data-used summary key, computed entirely
 * offline (bumelerze-engine/scripts/seed_atlas.py), zero network cost
 * at render time (D21: "Displayed maps are ALWAYS
 * bumelerze-engine products").
 */
