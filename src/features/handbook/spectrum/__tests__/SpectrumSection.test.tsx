...
describe("SpectrumSection", () => {
  ...
  it("shows the form and the honesty banner, but no chart/table, before Ss/S1 are entered", async () => {
    ...
    await render(<SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={NO_ISC2025} lat={35.56} lon={45.43} locale="en" />);
  ...
  it("renders end-to-end in ckb (Sorani, RTL) with no crash and shows the notation symbols untranslated", async () => {
    ...
    await render(<SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={NO_ISC2025} lat={35.56} lon={45.43} locale="ckb" />);
  ...
  /**
   * Previously this asserted the chart was BLANK until a layout event
   * arrived, which encoded a fragility as a requirement: where `onLayout`
   * never fires with a width — a hidden or zero-width container — the chart
   * rendered nothing at all, with no error and no fallback. It now starts
   * from a window-derived width and lets `onLayout` refine it.
   */
  it("draws immediately, then refines its width from the layout event", async () => {
    await i18n.changeLanguage("en");
    await render(<SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={NO_ISC2025} lat={35.56} lon={45.43} locale="en" />);
    ...
    // Drawn before any layout event ever arrives.
    ...
    // A zero measurement means "not laid out yet", not "no space", and must
    // not blank a chart that was drawing.
    await fireEvent(container, "layout", { nativeEvent: { layout: { x: 0, y: 0, width: 0, height: 0 } } });
    expect(screen.getByTestId("spectrum-code-curve")).toBeTruthy();
  });
});
...
describe("SpectrumSection pre-filled from the Iraqi Seismic Code 2025", () => {
  ...
  it("computes a spectrum immediately, with no typing at all", async () => {
    await i18n.changeLanguage("en");
    await render(
      <SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={DERBENDIKHAN} lat={35.56} lon={45.43} locale="en" />,
    );
  ...
  it("leaves the form empty where the code has no coverage", async () => {
    await i18n.changeLanguage("en");
    await render(
      <SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={NO_ISC2025} lat={35.56} lon={45.43} locale="en" />,
    );
  ...
  it("renders the pre-filled path in ckb (Sorani, RTL) without crashing", async () => {
    await i18n.changeLanguage("ckb");
    await render(
      <SpectrumSection vs30MS={VS30_D_BAND} ec8GroundType="C" isc2025={DERBENDIKHAN} lat={35.56} lon={45.43} locale="ckb" />,
    );
