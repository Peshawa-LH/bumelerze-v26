// VENDORED — hand-synced copy of
// `src/lib/felt-aggregation/cell-aggregation.ts`. See `answer-types.ts` in
// this folder for the vendoring rationale/sync story. Only mechanical
// differences from the source file: relative imports carry Deno-required
// `.ts` extensions. No aggregation-math changes.
...
import { CARTOON_TO_INTENSITY, scoreTier2Report } from "./cdi-scoring.ts";
import type {
...
/**
 * §3.2 step 2: CWS = Sigma(mean_index x weight). An index with no answering
 ...
 *
 * AMBIGUITY FLAG (documented in the source lib's README, not just here):
 ...
 * the letter of R14 anyway, because the gate's evidence-counting logic is
 * exactly what a future uncapped intensity value (most likely `ems_int`,
 * §3.4, out of scope this wave) will need.
 */
