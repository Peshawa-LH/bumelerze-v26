// VENDORED — hand-synced copy of `src/lib/felt-aggregation/cdi-scoring.ts`.
// See `answer-types.ts` in this folder for the vendoring rationale/sync
// story. Only mechanical differences from the source file: the
// `@/features/felt/types` import is replaced by the local
// `./answer-types.ts`, and relative imports carry Deno-required `.ts`
// extensions. No scoring-table/logic changes.
...
// update wave (owner directive, `answer-types.ts`'s `BuildingDamageGrade`)
// — the index VALUES per grade are unchanged, only the key domain shifted
// (old key 0 -> 1, ... old key 4 -> 5).
